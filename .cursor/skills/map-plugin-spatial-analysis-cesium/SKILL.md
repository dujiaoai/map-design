---
name: map-plugin-spatial-analysis-cesium
description: >-
  React 地图「Cesium 空间分析」能力（类型 cesium-toolkit）。需 MapHostProvider 宿主注入；实现前读 map-workspace-host-react。
---

# React：Cesium 空间分析（可视域/通视）
...
- 可视域分析 VisualfieldAnalysisCesiumEntry
- 通视分析 VisibilityAnalysisCesiumEntry
- 互斥激活同一时刻一种分析
...
| 概念 | React |
|------|-------|
| `useSpatialAnalysisCesium(caps)` | `useSpatialAnalysisCesium()` + MapHostProvider |
| visibleRef 按 mapId | Context / Zustand |

### 推荐目录

```
src/features/spatial-analysis-cesium/
├── hooks/useSpatialAnalysisCesium.ts
├── components/
├── core/
└── index.ts
```
