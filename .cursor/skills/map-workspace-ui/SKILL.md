...
完整树见 [map-workspace-ui.md](../../docs/architecture/map-workspace-ui.md) §页面挂载顺序。

**`workspace-canvas` 内（自底向上）：**

1. `WorkspaceMapAtmosphere` — 背景氛围
2. `MapCanvasContextMenu` — 右键菜单
3. `MapPlaceholder` — 地图引擎占位
4. `MapQuickToolbar` — 方案 C 快捷 `map-tool`
5. `MapControls` — map-chrome（罗盘/缩放/图例图层）
6. `MockMapToolHost` — movable-panel / anchor / panel
7. `MapToolDrawerPanel` — L4 drawer 条带
8. `MapNativeModuleHost` — display 模块
9. `MapContextPanelEdge` — 左列收起后的唯一展开条

**并列：** `MapContextPanel` 与 canvas 同在 `workspace-main` flex 行。

**Provider 外：** `AccountSheet`、`NotificationSheet`、`WorkspaceCommandPalette`。

**无 UI 同步：** `MapWorkspaceUrlSync`、`MapToolLifecycleSync`、`MapWorkspaceKeyboardSync`（在 `SidebarProvider` 内）。
...
| 地图 chrome | `apps/web/app/widgets/map-controls/` |
| 首页编排 | `apps/web/app/routes/home.tsx` |
