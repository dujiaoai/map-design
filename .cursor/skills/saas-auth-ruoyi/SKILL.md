...
description: >-
  saas-web 认证与会话：C-06～C-12 SaaS 主路径 ✅（含 C-09 tenant features 侧栏过滤）；
  Sprint D 权限与 admin。Use when adding protected routes,
...
| C-09 | ✅ `filterNavMainItemsForTenant` + `useEnabledTenantFeatures`（侧栏/命令面板） |
| C-10 | ✅ Account UI → `users/me*` |
| C-11 | ✅ TeamSwitcher → `GET /v1/tenants` |
| C-12 | ✅ — 已移除 `ruoyi-profile-store`；`useWorkspaceSession` |
| D-09 | ✅ — `requirePermission` / `PermissionCodes`；去 RuoYi `*:*:*` |
...
| 侧栏导航 | `mock-nav-items` + `filterNavMainItemsForTenant`（C-09 ✅） | RuoYi `getMenuRouters` |
...
4. 需权限：`requirePermission()` / `hasPermission()`（来自 `user.permissions[]`，见 `@repo/auth` `PermissionCodes`）
5. 工作台壳层已要求 `workspace:use`（`requireWorkspaceAccess`）
6. 新 SaaS 数据：在 `shared/queries/` 加 query options，经 `@repo/api-client`
7. 401/403：走 `clearAppSession()`，勿只清 localStorage 一半
...
- 细粒度权限码、`requirePermission` — ✅ D-09 对齐 `/v1/users/me` 与 `PermissionCodes`
