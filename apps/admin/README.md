...
| **列配置** | 用户自定义可见列 | ✅ 四主表 + 计费钱包/充值订单/积分流水/发票 localStorage 持久化 |
...
后续产品 backlog 见下方「Later（P4 余项）」。技术延续：`entities/` 已拆 tenant / user / audit-log / member / permission / role，其余域可按需迁移。
