...
pnpm validate:admin-e2e   # 根目录：Playwright mock 全量（与 CI 一致）
pnpm --filter @repo/saas-admin test:e2e   # 同上
...
**Phase 12（规模化运维）** ✅：SAML metadata 导入与 SP 证书轮换、SCIM Groups + 增量 sync、审计 Webhook SLA 大盘、Usage CSV 导出 + billing 对账、对象存储跨区复制与合规保留。
...
**Phase 16（智能运维深化）** ✅：SAML IdP 健康仪表盘与断连演练、SCIM 双向同步变更预览、Webhook 多区域归档摘要、FinOps 预算告警横幅、对象存储跨区域一致性校验（ADR-0018）。
