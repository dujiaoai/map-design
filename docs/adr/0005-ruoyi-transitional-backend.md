...
| 阶段 | 范围 | 策略 |
| --- | --- | --- |
| Sprint A/B | Auth、租户 API 后端 | 已完成；工作台仍临时走 RuoYi |
| **Sprint C** | **身份与会话** | **注册**、登录、用户信息、bootstrap **迁至 SaaS**；菜单 **mock-nav**；**不留** RuoYi 会话 API |
...
- 短期内维护两套 client 包
- Sprint C 实施前工作台仍临时依赖 RuoYi（迁移窗口）
- 文档需区分「当前实现（RuoYi）」与「Sprint C 目标（SaaS + mock-nav）」
