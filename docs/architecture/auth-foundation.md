...
| 项 | 问题 | 目标 |
| --- | --- | --- |
| A-01 | 停用租户登录与错误账号同为 401 | 明确 **403** + `Tenant is suspended` |
| A-02 | 禁用账号与密码错误同为 401 | 密码正确时 **403** + `Account is disabled` |
| A-03 | 前端 403 被通用文案覆盖 | RFC 7807 `detail` 优先展示 + 中文本地化 |
| A-04 | 密码规则分散在多处 `min(8)` | `@repo/auth` 统一常量与 Zod schema |
| A-05 | Admin 登录无「记住密码」选项 | 与 Web 行为对齐（可选保存） |
...
| 项 | 说明 |
| --- | --- |
| C-01 | Refresh 轮换边界：并发 refresh、过期 access 仍可用 `/users/me` | ✅ 30s 宽限期幂等 refresh；access JWT  Stateless 窗口见代码注释 |
...
- C-09 侧栏 `filterNavByTenant` / features 门控
...
- [x] `format-auth-api-error`：`detail` 优先 + 登录页中文映射
