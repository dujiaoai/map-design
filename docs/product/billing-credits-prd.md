...
| GET | `/v1/admin/billing/tenants/{tenantId}/ledger` | `?userId` 指定用户流水 |
...
| GET | `/v1/admin/billing/stats` | `?tenantKind` 统计 |
...
| **403** + suspended | 租户停用 | 全站拦截，禁止充值与消费 |

api-client 层统一 ProblemDetail 解析；地图/机库插件 **禁止** 各自解析 402。

### 5.2 apps/admin

改造 `apps/admin/app/routes/billing.tsx`：订单 / 钱包 / SKU / 调账 / 用量汇总；`tenantKind` 筛选；按 `productCode` GMV
...
| `sys_tenant.status=suspended` | 禁止充值与扣费（403） |
...
| internal API 泄露 | 内网 + `BILLING_INTERNAL_TOKEN`；hold 必带真实 principal |
| Flyway 冲突 | billing 表仅 billing-api；权限 V18 仅 saas-api |
| suspended 租户 | 禁止充值与扣费（403） |
