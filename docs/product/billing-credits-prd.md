...
> 状态：F-1～F-6 骨架 **已落地**（2026-06-15）；**安全/限流/观测加固** ✅（落地索引见 [billing-service.md](../architecture/billing-service.md) §Sprint F 安全与稳定性改进）；billing **可选独立 DB 骨架** ✅  
...
- **JSAPI OAuth**：`GET /v1/billing/wechat/oauth/config`；`GET /v1/billing/wechat/oauth/openid?code=`；下单可选 `wechatOpenId`

| 配置 | 说明 |
| --- | --- |
| `billing.payment.provider-mode` | `stub` \| `live` |
| `billing.payment.wechat.*` | `app-id`、`app-secret`（OAuth）、`mch-id`、`api-v3-key`、`merchant-serial-no`、`private-key-pem`、`notify-url`、`default-pay-scene` |
...
- **优惠券**（骨架 ✅）：
  ...
  - **叠加规则骨架**（`BillingCouponStackingPolicy`）：`billing.coupon.stacking-mode`（默认 `single`）、`max-coupons-per-recharge`、`min-payable-cents-after-discount`、`block-concurrent-pending-recharge-coupons`；充值下单前校验
  - **不含**多券组合算法、跨订单券池（后续迭代）
...
- 可选 billing 独立 DB（骨架 ✅）：`copy` / `api` / **`cdc` pull + push 兜底**（`SAAS_BILLING_MEMBERSHIP_PUSH_ENABLED` + `BILLING_MEMBERSHIP_PUSH_RECEIVE_ENABLED`）
...
| sec | 安全与稳定性加固（Webhook/Caller/RFC7807/Job/CDC push·HMAC/Admin 分页/PostgreSQL RLS/冒烟 24 步 mock） | ✅ 2026-06-14 |
...
| f-sec-hardening | Webhook 验签/限流、Internal Caller、RFC7807、Micrometer、Admin ledger/分页、membership push·HMAC、signup/refund/订单过期 Job、**PostgreSQL RLS**、`SuspendedTenantBillingPolicyTest`、冒烟 24 步（mock） ✅（2026-06-14）；详见 [billing-service.md](../architecture/billing-service.md) §Sprint F 安全与稳定性改进、[billing-tenant-rls.md](../architecture/supplements/billing-tenant-rls.md) |
