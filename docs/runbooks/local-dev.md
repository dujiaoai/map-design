...
- Flyway 在启动时自动执行 `db/migration/V*.sql` 与 PostgreSQL 专用迁移（saas-api：`migration-postgresql/V5__rls.sql`；billing-api：`migration-postgresql/V12__billing_tenant_rls.sql`）；原理见 [tenant-rls-b05.md](../architecture/supplements/tenant-rls-b05.md) 与 [billing-tenant-rls.md](../architecture/supplements/billing-tenant-rls.md)
