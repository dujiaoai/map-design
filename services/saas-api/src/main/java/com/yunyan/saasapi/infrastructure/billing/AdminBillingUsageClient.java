...
public class AdminBillingUsageClient {
      ...
      var response =
          restTemplate()
              .exchange(
                  ...
                  EventCountResponse.class);
      if (response.getBody() == null) {
        return 0L;
      }
      return Math.max(response.getBody().eventCount(), 0L);
    } catch (RestClientException ex) {
      log.debug("Billing usage fetch failed: {}", ex.getMessage());
      return 0L;
    }
  }
  ...
  record EventCountResponse(long eventCount) {}
}
